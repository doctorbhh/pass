...
(function () {
  ...
  if (primaryModifierT && event.shiftKey && event.code === "KeyT") {
      ...
      // which calls window._passStartTyping(code) directly.
      return;
    }
  ...
  window._passStartTyping = function(codeToType) {
    if (!codeToType) return;
    console.log('[exam.js] _passStartTyping called, length:', codeToType.length);
    const found = findAnswerEditor();
    if (found) {
      try {
        editor = found;
        ...
        console.error('[exam.js] Error in _passStartTyping:', error);
      }
    } else {
