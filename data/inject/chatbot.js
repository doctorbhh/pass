...
(function() {
    chrome.storage.local.get(['stealth', 'chatIconVisible'], function(result) {
        ...
        const isChatIconVisible = result.chatIconVisible !== false;
        console.log("Initial stealth mode state:", isStealthModeEnabled);
        console.log("Initial chat icon visibility:", isChatIconVisible);
        ...
        if (!isChatIconVisible) {
            const hostBtn = document.getElementById("chat-button-shadow-host");
            const hostOverlay = document.getElementById("chat-overlay-shadow-host");
        ...
        chrome.storage.onChanged.addListener((changes, namespace) => {
            if (namespace === 'local') {
                ...
                if (changes.chatIconVisible) {
                    const isVisible = changes.chatIconVisible.newValue !== false;
                    const hostBtn = document.getElementById("chat-button-shadow-host");
                    const hostOverlay = document.getElementById("chat-overlay-shadow-host");
                    if (hostBtn) hostBtn.style.display = isVisible ? "block" : "none";
                    if (hostOverlay) {
                        if (!isVisible) {
                            // Close overlay if open when toggling off
                            const overlayElement = hostOverlay._shadowRoot?.getElementById("chat-overlay");
