                ...
                try {
                    await chrome.scripting.executeScript({
                        ...
                        func: function(msg, version) {
                            ...
                            gradientContainer.id = 'pass-update-notification';
                            ...
                            // Header container with Pass title and close button
                            ...
                            title.innerHTML = 'Pass Extension';
                            title.style.cssText = `
                                font-size: 16px;
                                font-weight: bold;
                                background: linear-gradient(to right, #3b82f6, #8b5cf6, #ec4899);
                            ...
                            const existingToast = document.getElementById('pass-update-notification');
                            if (existingToast) {
...
function handleQueryResponseForIamNeoExamly(response, tabId, isMCQ = false, isHackerRank = false, isMultipleChoice = false, isTyped = false) {
    if (response && typeof response === 'string') {
        ...
        if (isMCQ) {
            ...
            if (isTyped) {
                // Typed mode: call _passStartTyping to type character-by-character
                chrome.scripting.executeScript({
                    target: { tabId: tabId },
                    func: function(code) {
                        console.log('[INJECTED] Calling _passStartTyping, code length:', code.length);
                        if (typeof window._passStartTyping === 'function') {
                            window._passStartTyping(code);
                        } else {
                            console.error('[INJECTED] _passStartTyping not found on window');
                        }
                    },
                    args: [cleanedCode],
                    world: 'MAIN'
...
async function queryRequest(text, isMCQ = false, isMultipleChoice = false, tabId = null) {
    ...
    try {
        ...
        prompt += "\n\nIMPORTANT: If your response includes any code, do NOT add any comments inside the code. No inline comments, no block comments, no documentation comments. Only provide clean code without any comments.";

        if (customAPIConfig.useCustomAPI && customAPIConfig.apiKey) {
            const result = await queryCustomAPI(prompt, isMCQ, isMultipleChoice, customAPIConfig);
            unblockRequests();
            return result;
        }

        // Web Automator fallback
        const provider = await getFreeAIProvider();
        let answer;
        
        if (provider === 'chatgpt') {
            answer = await askChatGPTOffscreen(prompt);
        } else {
            answer = await askGeminiOffscreen(prompt);
        }
...
async function handleChatMessage(message, sender) {
    try {
        ...
        const chatPrompt = message.context 
            ? `Context: ${message.context}\n\nUser: ${message.message}\n\nPlease provide a helpful response. IMPORTANT: If your response includes any code, do NOT add any comments inside the code. No inline comments, no block comments, no documentation comments. Only provide clean code without any comments.`
            : message.message + "\n\nIMPORTANT: If your response includes any code, do NOT add any comments inside the code. No inline comments, no block comments, no documentation comments. Only provide clean code without any comments.";
        ...
        if (provider === 'chatgpt') {
            answer = await askChatGPTOffscreen(chatPrompt);
        } else {
            answer = await askGeminiOffscreen(chatPrompt);
        }
        
        if (answer) {
            sendChatResponse(sender.tab.id, answer);
        } else {
            sendChatErrorResponse(sender.tab.id, 'No response received from the AI provider.');
        }

    } catch (error) {
        console.error("Chat processing error:", error);
        sendChatErrorResponse(sender.tab.id, "Sorry, I encountered an error processing your message.");
    }
}
...
function showOpacityLevelToast(tabId, message) {
    ...
    chrome.scripting.executeScript({
        ...
        func: function(msg, opacityLevel) {
            ...
            const toast = document.createElement('div');
            toast.id = 'pass-active-toast'; // Add ID for tracking
...
async function showStealthToast(tabId, message, stealthEnabled) {
    ...
    chrome.scripting.executeScript({
        ...
        func: function(msg, stealthEnabled, opacity) {
            // Create toast container
            const toast = document.createElement('div');
            toast.id = 'pass-active-toast'; // Use same ID for tracking
...
// ==========================================
// AI WEB AUTOMATORS (Gemini & ChatGPT)
// ==========================================

async function getFreeAIProvider() {
    return new Promise((resolve) => {
        chrome.storage.local.get(['freeAIProvider'], (result) => {
            resolve(result.freeAIProvider || 'gemini');
        });
    });
}
...
async function getOrCreateGeminiTab() {
    return new Promise((resolve) => {
        chrome.tabs.query({ url: "*://gemini.google.com/*" }, (tabs) => {
            if (tabs.length > 0) {
                // Make sure existing tab is fully loaded
                ...
                chrome.tabs.create({ url: "https://gemini.google.com/app", pinned: true, active: false }, (tab) => {
                    // Wait for the new tab to fully load
                    const listener = (tabId, changeInfo) => {
                        if (tabId === tab.id && changeInfo.status === 'complete') {
                            chrome.tabs.onUpdated.removeListener(listener);
                            // Extra delay for Gemini's JS to initialize
                            setTimeout(() => {
                                chrome.tabs.get(tabId, (t) => resolve(t));
                            }, 3000);
                        }
...
async function askGeminiOffscreen(prompt) {
    const tab = await getOrCreateGeminiTab();
    
    return new Promise((resolve, reject) => {
        // Step 1: Inject text into the editor
        chrome.scripting.executeScript({
            target: { tabId: tab.id },
            world: 'MAIN',
            func: async (textToInject) => {
                try {
                    // Helper: find the actual editable element (traverses shadow DOM)
                    function findEditor() {
                        // Strategy 1: rich-textarea shadow DOM (current Gemini)
                        const richTextarea = document.querySelector('rich-textarea');
                        if (richTextarea && richTextarea.shadowRoot) {
                            const inner = richTextarea.shadowRoot.querySelector('.ql-editor, [contenteditable="true"]');
                            if (inner) return inner;
                        }
                        // Also try the rich-textarea itself
                        if (richTextarea) {
                            const nested = richTextarea.querySelector('.ql-editor, [contenteditable="true"]');
                            if (nested) return nested;
                        }
                        
                        // Strategy 2: direct .ql-editor
                        const qlEditor = document.querySelector('.ql-editor[contenteditable="true"]');
                        if (qlEditor) return qlEditor;
                        
                        // Strategy 3: any contenteditable in the input area
                        const inputArea = document.querySelector('.input-area, .prompt-container, .text-input-field');
                        if (inputArea) {
                            const ce = inputArea.querySelector('[contenteditable="true"]');
                            if (ce) return ce;
                        }
                        
                        // Strategy 4: generic contenteditable (pick the last one, usually the input)
                        const allCE = document.querySelectorAll('[contenteditable="true"]');
                        if (allCE.length > 0) return allCE[allCE.length - 1];
                    ...
                    // Wait for editor to appear (up to 10 seconds)
                    let editor = null;
                    for (let i = 0; i < 20; i++) {
                        editor = findEditor();
                        if (editor) break;
                    ...
                    if (!editor) {
                        return "Error: Could not find Gemini editor after waiting. Make sure you are logged into gemini.google.com";
                    }
                    ...
                    try {
                        ...
                        sel.collapseToEnd();
                        injected = document.execCommand('insertText', false, textToInject);
                    } catch(e) {}
                    
                    // Verify injection worked
                    const currentText = editor.tagName.toLowerCase() === 'textarea' 
                        ? editor.value 
                        : editor.textContent;
                    
                    if (!injected || currentText.trim().length === 0) {
                        // Method 2: Direct DOM + input event
                        if (editor.tagName.toLowerCase() === 'textarea') {
                            const nativeSet = Object.getOwnPropertyDescriptor(
                                HTMLTextAreaElement.prototype, 'value'
                            ).set;
                            nativeSet.call(editor, textToInject);
                        } else {
                            while (editor.firstChild) editor.firstChild.remove();
                            const p = document.createElement('p');
                    ...
                    // Wait a moment for UI to update and enable send button
                    ...
                    function findSendButton() {
                        ...
                        const selectors = [
                            ...
                            '.input-area button[aria-label*="Send"]',
                        ...
                        // Last resort: find any enabled button near the input
                    ...
                    return new Promise((res) => {
                        ...
                        const responseSelectors = 'model-response, message-content, .response-container, .model-response-text, .markdown-main-panel';
                        const initialCount = document.querySelectorAll(responseSelectors).length;
                        
                        const checkInterval = setInterval(() => {
                            attempts++;
                            
                            // Detect if generation has started
                            const loading = document.querySelector(
                                '.loading-indicator, generating-indicator, .generating, ' +
                                '[aria-label="Stop generating"], .stop-button, mat-spinner'
                            );
                            if (loading) generationStarted = true;
                            
                            const currentResponses = document.querySelectorAll(responseSelectors);
                            ...
                            if (currentResponses.length > initialCount) {
                                ...
                                // Check if generation is complete (no loading indicators)
                                const stillLoading = document.querySelector(
                                    '.loading-indicator, generating-indicator, .generating, ' +
                                    '[aria-label="Stop generating"], .stop-button, mat-spinner'
                                );
                                
                                if (!stillLoading && text.length > 0) {
                                    // Wait a bit more for any final rendering
                                    setTimeout(() => {
                                        clearInterval(checkInterval);
                                        const finalText = (latest.innerText || latest.textContent || '').trim();
                                        res(finalText);
                                    }, 2000);
                                    return;
                                }
                            ...
                            if (attempts > 120) { // 60 seconds
                                clearInterval(checkInterval);
                                // Try to grab whatever response exists
                                const allResp = document.querySelectorAll(responseSelectors);
                                if (allResp.length > initialCount) {
                                    const last = allResp[allResp.length - 1];
                                    res((last.innerText || last.textContent || '').trim() || 
                                        "Response timed out. The response may still be generating on the Gemini tab.");
                                } else {
                                    res("Request timed out. Please make sure you are logged into gemini.google.com and try again.");
                                }
                            }
                        }, 500);
                    });
            ...
            if (chrome.runtime.lastError || !results || !results[0]) {
                reject(chrome.runtime.lastError || new Error("Failed to execute script on Gemini tab"));
            } else {
    ...
    return new Promise((resolve) => {
        chrome.tabs.query({ url: "*://chatgpt.com/*" }, (tabs) => {
            if (tabs.length > 0) {
                ...
                chrome.tabs.create({ url: "https://chatgpt.com/", pinned: true, active: false }, (tab) => {
                    resolve(tab);
                });
            }
        });
...
async function askChatGPTOffscreen(prompt) {
        ...
        chrome.scripting.executeScript({
            ...
            func: async (textToInject) => {
                try {
                    ...
                    if (!editor) throw new Error("Could not find ChatGPT editor");
                    
                    editor.focus();
                    
                    // Simulate input event to update ProseMirror state
                    ...
                    return new Promise((res) => {
                        ...
                        const checkInterval = setInterval(() => {
                            attempts++;
                            // Check if generation is happening (stop-button is visible)
                            const stopBtn = document.querySelector('[data-testid="stop-button"]');
                            
                            // If generation started and then stop button disappeared, it's done
                            if (!stopBtn && attempts > 4) {
                                const responses = document.querySelectorAll('.markdown');
                                if (responses.length > 0) {
                                    clearInterval(checkInterval);
                                    const latest = responses[responses.length - 1];
                                    res(latest.innerText || latest.textContent);
                                }
                            }
                            
                            if (attempts > 60) { // 30 seconds max
                                clearInterval(checkInterval);
                                res("Request timed out. Please try again.");
                            }
                        }, 500);
                    });
                } catch (e) {
            ...
            if (chrome.runtime.lastError || !results || !results[0]) {
                reject(chrome.runtime.lastError || new Error("Failed to execute script"));
            } else {
                resolve(results[0].result);
            }
        });
